...
eraser_img = cv2.imread(r'C:\Users\111fa\Downloads\Eraser.png')
clear_img = cv2.imread(r'C:\Users\111fa\Downloads\Clear all.png')

# Check if images are loaded properly
if eraser_img is None:
    print("Error loading eraser image. Check the path.")
    exit()
if clear_img is None:
    print("Error loading clear image. Check the path.")
    exit()
...
def animatedBrushStroke(x1, y1, xp, yp, drawColor):
    # Add a glowing effect to the brush
    for i in range(5):
        offset_x = np.random.randint(-10, 10)
        offset_y = np.random.randint(-10, 10)
        cv2.circle(img, (x1 + offset_x, y1 + offset_y), 10, drawColor, cv2.FILLED)
        cv2.circle(imgCanvas, (x1 + offset_x, y1 + offset_y), 10, drawColor, cv2.FILLED)

def animatedEraser(x1, y1):
    # Simple bouncing effect for eraser
    eraser_x = int(x1 + np.sin(cv2.getTickCount() / 100.0) * 20)
    eraser_y = int(y1 + np.cos(cv2.getTickCount() / 100.0) * 20)
    cv2.circle(img, (eraser_x, eraser_y), 25, (0, 0, 0), -1)
    cv2.circle(imgCanvas, (eraser_x, eraser_y), 25, (0, 0, 0), -1)

def confettiBrush(x1, y1, drawColor):
    for _ in range(10):
        offset_x = np.random.randint(-20, 20)
        offset_y = np.random.randint(-20, 20)
        cv2.circle(img, (x1 + offset_x, y1 + offset_y), 5, drawColor, -1)
        cv2.circle(imgCanvas, (x1 + offset_x, y1 + offset_y), 5, drawColor, -1)

def partyModeBrush(x1, y1, xp, yp):
    colors = [(np.random.randint(255), np.random.randint(255), np.random.randint(255)) for _ in range(5)]
    for color in colors:
